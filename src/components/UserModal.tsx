...
const UserModal: React.FC<UserModalProps> = ({ user, isOpen, onClose }) => {
  ...
  const formatDate = (
    date: string | { _seconds: number } | Date | undefined
  ) => {
    if (!date) return "N/A";
    try {
      const dateObj =
        typeof date === "object" && date !== null && "_seconds" in date
              ...
              {propertiesLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    This user hasn't visited any properties yet
                  </p>
                </div>
              ) : (
                        ...
                        {(() => {
                          ...
                          if (
                            ...
                            const photos = property.photos
                              .filter((photo: string | { url: string }) => {
                                const photoUrl =
                                  typeof photo === "string"
                                    ? photo
                                    : photo?.url;
                                return (
                                  photoUrl &&
                                  photoUrl.trim() &&
                                  !availableImages.includes(photoUrl)
                                );
                              })
                              .map((photo: string | { url: string }) =>
                          ...
                          if (
                            property.allPhotos &&
                            Array.isArray(property.allPhotos)
                          ) {
                            const allPhotos = property.allPhotos.filter(
                              (photo: string) =>
                                photo &&
                                photo.trim() &&
                                !availableImages.includes(photo)
                            );
                          ...
                          if (
                            ...
                            property.image.trim() &&
                            !availableImages.includes(property.image)
                          ) {
                        ...
                        <div className="flex items-center mt-2 text-xs text-gray-400">
                          ...
                          {activity.date && activity.time && (
                            <>
                              <span className="mx-2">•</span>
                              <span>
                                Visit: {activity.date} at {activity.time}
                              </span>
                            </>
                          )}
                        </div>
