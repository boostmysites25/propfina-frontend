...
interface RecentlyAddedPropertiesProps {
  selectedCity: string;
  onAddProperty: () => void;
}

const RecentlyAddedProperties: React.FC<RecentlyAddedPropertiesProps> = ({ selectedCity, onAddProperty }) => {
  const [properties, setProperties] = useState([
    {
      id: 1,
      title: "Recently Added flat",
      type: "buy",
      price: "₹ 53,444",
    ...
    <div className="px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">
          Recently Added Properties - {selectedCity}
        </h1>
        <button 
      ...
      {properties.length === 0 && (
        <div className="flex flex-col items-center justify-center h-96 bg-white rounded-lg shadow-sm p-8 text-center">
          ...
          <p className="text-gray-500 mb-6">
            There are no recently added properties for {selectedCity}.
          </p>
          <button 
            onClick={onAddProperty}
